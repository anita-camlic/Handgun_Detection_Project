...
Statistics from Everytown.com and Forbes' magazine show that in the United States there have been 704 mass shootings so far this year (2022).  In the United States, the rate of gun deaths has **increased 33%** since 2011, the rate of gun homicide **increased 70%** since 2011.  As of Nov. 25 2022,  **39,935**  people have died from gun-related events in the United States. this year, including **18,221** who died by homicide, murder or an unintentional shooting, plus another 21,714 who died by suicide.
...
This model could be part of a security system. It could trigger an alarm, a call to the police, or even initiate the evacuation and closing of an area. This can be used as a first line of defense against mass shooters. However, this should not be the only sort of defense. This is simply a tool that can quickly and efficiently classify a gun vs other handheld objects. The main point of this system is to save time, and to get help to the area sooner in hopes to mitigate any potential damage.

Note: Though this model is directed towards public school districts,but it is not limited to schools. I would like to see it implemented in all public spaces like grocery stores, government buildings, banks, shopping malls, places of worship, and more.
...
We conclude with an accuracy score of `0.7878189253807068` and a loss of `1.718071460723877` on the test data. Unfortunately, these models did not work as well as I would have wished on unseen data. However, I do believe that with more time and more diverse data, I will be able to increase the accuracy score. It is truly important that the accuracy score is as high as it can be for what it's intended use is.  

The final recommendations are to use this model as a tool for surveillance on the entrances and parking lot of public schools. This model has the power to classify images as either containing a weapon or not. This ability is so important to keeping children, teachers and the general public safe.  
...
Another issue my model ran into was merging the background and foreground of an image when they should have been kept seperate. In the future I will explore changing contrast of the images to see if it will make it easier for my model to differentiate the foreground from the background.
...
I also have an interest in augmenting the data I already have in hopes that this will help the model discern the foreground from the background.
...
Please review my full analysis & modeling history in my Jupyter Notebook or my presentation.
